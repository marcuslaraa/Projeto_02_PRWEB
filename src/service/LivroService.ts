...
export class LivroService {
  ...
  async inserirLivro(livro: any): Promise<Livro> {
    const existeCategoria =
      await this.categoriaRepository.consultarCategoriaPorID(livro.categoriaId)
    if (!existeCategoria) throw new Error("Não existe categoria.")
    const consultaLivro = await this.livroRepository.consultarLivroPorTitulo(
      livro.titulo
    )
  ...
  async atualizarLivroPorId(novoLivro: any, id: any): Promise<Livro | null> {
    const consultarId = await this.livroRepository.consultarLivroPorId(id)

    if (!consultarId)
      throw new Error("Não existe livro com esse id para ser atualizado.")
    const novaLivroAdicionado = await this.livroRepository.atualizarLivroPorId(
      novoLivro,
      id
    )
